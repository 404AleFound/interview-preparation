...
1. T3 启动事务，此时生成自己的 Read View，记录下当前活跃未提交事务（如 T2）。
2. T3 在整个事务期间，无论什么时候读取数据，看到的都是事务启动时的快照（即 T2 还未提交时的状态）。
3. 即使 T2 在 T3 事务期间提交了，T3 也无法看到 T2 的修改，直到 T3 提交或回滚。

**总结：**  
可重复读保证了同一事务内多次读取同一数据结果一致，避免了不可重复读问题。
## 读提交的原理
...
1. T3 启动事务，此时还未读取数据。
2. T2 在 T3 事务期间提交了新的数据。
3. T3 第一次读取数据时，生成一个新的 Read View，此时能看到 T2 已提交的数据。
4. 如果 T3 再次读取同一数据，会再次生成新的 Read View，看到的是最新已提交的数据。

**总结：**  
读提交隔离级别下，每次读取都能看到其他事务已提交的最新数据，因此可能出现同一事务内多次读取结果不一致的“不可重复读”现象。